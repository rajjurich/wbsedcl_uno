<%@ Page Title="" Language="C#" MasterPageFile="~/ModuleMain.master" AutoEventWireup="true"
    CodeBehind="SACViewCard.aspx.cs" Inherits="UNO.SACViewCard" %>
    ...
    <script type="text/javascript">
      ...
      116, 42, 200, 150, 21, 75, 169, 247, 182, 232, 10, 84, 215, 137, 107, 53];
        ...
        window.onload = function () {
            document.getElementById("<%= pnlViewCard.ClientID %>").style.display = "none";
        };
        ...
        function ViewCard() {
            ...
            try {
                ...
                $.ajax({
                    url: "SACViewCard.aspx/GetMasterKey",
                    type: "POST",
                    dataType: "json",
                    data: "",
                    async: false,
                ...
                //                $.ajax({
                //                    url: "SACViewCard.aspx/GetDOSKey",
                //                    type: "POST",
                //                    dataType: "json",
                //                    data: "",
                //                    async: false,
                ...
                data = objReadCard.LoadKey(MasterKey);
                if (data != "") {
                    alert("Unable to Load key to Block 59");
                    return false;
                }
                ...
                data = objReadCard.ReadBlock(55).replace(/ /g, "");
                //  var key = data.substr(8,12);

                var Masterkey = objReadCard.ReadMasterKey(111111, 111111);
                alert("Step1"+Masterkey);
                ...
                if (data != "") {
                    alert("Unable to Authenticate Block 04");
                    return false;
                }
                data = objReadCard.LoadKey(CMSCalulatedKey);
                if (data != "") {
                    alert("Unable to Load key to Block 04");
                    return false;
                }
                data = objReadCard.ReadBlock(4).replace(/ /g, "");
                EmpNameHex = data;
                data = objReadCard.Authenticate(5, CMSCalulatedKey, 96);
                if (data != "") {
                    alert("Unable to Authenticate Block 05");
                    return false;
                }
               
                data = objReadCard.LoadKey(CMSCalulatedKey);
                if (data != "") {
                    alert("Unable to Load key to Block 05");
                    return false;
                }

                data = objReadCard.ReadBlock(5).replace(/ /g, "");
                EmpNameHex = EmpNameHex + data;


                data = objReadCard.Authenticate(6, CMSCalulatedKey, 96);
                if (data != "") {
                    alert("Unable to Authenticate Block 06");
                    return false;
                }

                data = objReadCard.LoadKey(CMSCalulatedKey);
                if (data != "") {
                    alert("Unable to Load key to Block 06");
                    return false;
                }

                data = objReadCard.ReadBlock(6).replace(/ /g, "");
                var block6 = data.substr(0, 22);
                var DOB = block6.substr(0, 8);
                DOB = DOB.substr(0, 2) + "/" + DOB.substr(2, 2) + "/" + DOB.substr(4, 4);
                var expiryTime = block6.substr(18, 4);
                expiryTime = expiryTime.substr(0, 2) + ":" + expiryTime.substr(2, 2);
                var gender = block6.substr(8, 2);

                if (gender == "01") {
                    $("select" + "#" + "<%=ddlGender.ClientID%>").prop('selectedIndex', 0);
                }
                else if  (gender == "02"){
                    $("select" + "#" + "<%=ddlGender.ClientID%>").prop('selectedIndex', 1);
                }
                  else  {
                    $("select" + "#" + "<%=ddlGender.ClientID%>").prop('selectedIndex', 2);
                }

                /// calculated key to unlock sector 2
            ...
            //   var key3 = objReadCard.CalculatetApplKey(parseInt(ReadData), RandomNo, Key_a_Buf);
             ...
             //   var Key3 = CalculateCMSKeyWithRandomNo("566216778359", "0000" + cardCSNR);
                alert("tmh"+Key3);
                data = objReadCard.Authenticate(8, Key3, 96);
                if (data != "") {
                    alert("Unable to Authenticate Block 8");
                    return false;
                }
                data = objReadCard.LoadKey(Key3);
                if (data != "") {
                    alert("Unable to Load key to Block 8");
                    return false;
                }
                data = objReadCard.ReadBlock(8).replace(/ /g, "");
                var EmployeeCode = data;
                var EmployeeCode = data.substr(0, 20);
              EmployeeCode = EmployeeCode.substr(0, 16);
                var empcode = "";
                var j = 1;
                for (var i = 0; i < EmployeeCode.length; i++) {
                    empcode += EmployeeCode.substr(j, 1);
                    j = j + 2;
                }
                ...
                var EmpCodePrefix = data.substr(14, 4);
                var EmpCodeNumber = data.substr(19, 5);
              ...
              //  alert(CMSCalulatedKey);
               ...
               // alert(data);
                debugger;
                var ExpiryDate = data.substr(12, 8);
                ExpiryDate = ExpiryDate.substr(0, 2) + "/" + ExpiryDate.substr(2, 2) + "/" + ExpiryDate.substr(4, 4);

               
                document.getElementById("<%= txtEmployeeCode.ClientID %>").value = empcode;
                document.getElementById("<%= txtDOB.ClientID %>").value = DOB;
                document.getElementById("<%= txtCardExpiryTime.ClientID %>").value = expiryTime;     
                document.getElementById("<%= txtEmployeeName.ClientID %>").value = HexToASCII(EmpNameHex);
                document.getElementById("<%= txtExpiryDate.ClientID %>").value = ExpiryDate;
                document.getElementById("<%= ddlEmployeeType.ClientID %>").value = (EmpType == "01") ? "P" : "T";


                document.getElementById("<%= pnlViewCard.ClientID %>").style.display = "";
                document.getElementById("<%= pnlReadCard.ClientID %>").style.display = "none";
                return false;
            }
            catch (ex) {
                alert(ex.Message);
        ...
        function ModifyCard() {
            try {
                ...
                //*********************************************************Change Name in Sector 1 End **************************************************** //
                //*********************************************************Change Employee Type, Expiry Date in Sector 10 Start **************************************************** //
                data = objReadCard.Authenticate(40, DosKey, 96);
                if (data != "") {
                    alert("Unable to Authenticate Block 40");
                    return false;
                }
                data = objReadCard.LoadKey(DosKey);
                if (data != "") {
                ...
                var DOE10 = document.getElementById("<%= txtExpiryDate.ClientID %>").value;
                ...
                WriteData = "";
                WriteData = data.substr(0, 6) + EmployeeType10 + date + month + year + data.substr(14);
                ...
                //*********************************************************Change Employee Type, Expiry Date in Sector 10 End **************************************************** //
                //*********************************************************Change Expiry Date in Sector 15 Start **************************************************** //
                data = objReadCard.Authenticate(60, CMSCalulatedKey, 96);
                if (data != "") {
                    alert("Unable to Authenticate Block 60");
                    return false;
                ...
                var DOE15 = document.getElementById("<%= txtExpiryDate.ClientID %>").value;
                DOE15 = DOE15.replace(/\//g, "");
                data = objReadCard.ReadBlock(60).replace(/ /g, "");
                WriteData = data.substr(0, 12) + DOE15 + data.substr(20);
                WriteData = objReadCard.Convert(WriteData);
                ...
                //*********************************************************Change Expiry Date in Sector 15 End **************************************************** //
                document.getElementById("<%= pnlViewCard.ClientID %>").style.display = "none";
                document.getElementById("<%= pnlReadCard.ClientID %>").style.display = "";
                return false;
            }
            catch (ex) {
                alert(ex.Message);
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <asp:Panel ID="pnlReadCard" runat="server" Style="width: 100%; text-align: center;">
        <table style="margin: 0 auto;">
            <tr>
                <td colspan="2" class="heading">
                    Please Place card to Read and Press OK
                </td>
            </tr>
            <tr>
                <td colspan="2" style="text-align: center;">
                    <asp:Button ID="btnSubmitPerso" runat="server" Text="OK" CssClass="ButtonControl"
                        OnClientClick="return ViewCard();" ValidationGroup="CardPerso" />
    ...
    <asp:Panel ID="pnlViewCard" runat="server" Style="width: 100%; text-align: center;">
        <table style="margin: 0 auto;">
            <tr>
                <td colspan="2" class="heading">
                    Card Details
            ...
            <tr>
                ...
                <td>
                    <asp:TextBox ID="txtEmployeeCode" runat="server" MaxLength="10" ReadOnly="true"
                        ValidationGroup="ModifyCard"></asp:TextBox>
                   <%-- <asp:RequiredFieldValidator ID="rfvtxtEmployeeCode" runat="server" ErrorMessage="Please enter Employee Code"
                        ControlToValidate="txtEmployeeCode" Display="None" ValidationGroup="ModifyCard"></asp:RequiredFieldValidator>
                    <ajaxToolkit:ValidatorCalloutExtender ID="vcerfvtxtEmployeeCode" runat="server" TargetControlID="rfvtxtEmployeeCode"
                        PopupPosition="Right">
            ...
            <tr>
                ...
                <td>
                    <asp:TextBox ID="txtEmployeeName" runat="server" ValidationGroup="ModifyCard"></asp:TextBox>
                <%--    <asp:RequiredFieldValidator ID="rfvtxtEmployeeName" runat="server" ErrorMessage="Please Enter Name"
                        ControlToValidate="txtEmployeeName" Display="None" ValidationGroup="ModifyCard"></asp:RequiredFieldValidator>
                    <ajaxToolkit:ValidatorCalloutExtender ID="vcerfvtxtEmployeeName" runat="server" TargetControlID="rfvtxtEmployeeName"
                        PopupPosition="Right">
                    </ajaxToolkit:ValidatorCalloutExtender>--%>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <asp:TextBox ID="txtDOB" runat="server" ValidationGroup="ModifyCard" ReadOnly="true"></asp:TextBox>
            ...
            <tr>
                ...
                <td>
                    <asp:TextBox ID="txtExpiryDate" runat="server" ValidationGroup="ModifyCard" ReadOnly="true"></asp:TextBox>
                    <ajaxToolkit:CalendarExtender ID="caltxtExpiryDate" runat="server" TargetControlID="txtExpiryDate"
                 ...
                 <%--   <asp:RequiredFieldValidator ID="rfvtxtExpiryDate" runat="server" ErrorMessage="Please enter Expiry Date"
                        ControlToValidate="txtExpiryDate" Display="None" ValidationGroup="ModifyCard"></asp:RequiredFieldValidator>
                    <ajaxToolkit:ValidatorCalloutExtender ID="vcerfvtxtExpiryDate" runat="server" TargetControlID="rfvtxtExpiryDate"
                        PopupPosition="Right">
                    </ajaxToolkit:ValidatorCalloutExtender>--%>
                </td>
            </tr>
               <tr>
                                            ...
                                            <td style="text-align: left;">
                                                <asp:TextBox ID="txtCardExpiryTime" runat="server" CssClass="TextControl" MaxLength="5"
                                                    ...
                                                    TabIndex="2" ValidationGroup="CardPerso" ReadOnly="true"></asp:TextBox>
                                            </td>
                                            <td>
                                            </td>
                                        </tr>
            ...
            <tr style="display:none;">
                ...
                <td>
                    <asp:DropDownList ID="ddlEmployeeType" runat="server" ValidationGroup="ModifyCard">
            ...
            <tr>
                <td colspan="2" style="text-align: center;">
                    <asp:Button ID="btnModifyCard" runat="server" Text="Modify" CssClass="ButtonControl"
                        OnClientClick="return ModifyCard();" ValidationGroup="ModifyCard" style="display:none;" />
                    <asp:Button ID="btnCancelModify" runat="server" Text="Cancel" CssClass="ButtonControl"
                        OnClientClick="return CancelModifyCard();" />
                </td>
            </tr>
        </table>
    </asp:Panel>
