<%@ Page Title="" Language="C#" MasterPageFile="~/ModuleMain.master" AutoEventWireup="true"
    CodeBehind="VisitorRawCard.aspx.cs" Inherits="UNO.VisitorRawCard" %>
      ...
      116, 42, 200, 150, 21, 75, 169, 247, 182, 232, 10, 84, 215, 137, 107, 53];
        ...
        function RawCard() {
            try {
                ...
                success: function (msg) {
                if (msg.d == "") {
                alert("Dos Key not found. Please Verify Dos card first.");
                ServerFlag = false;
                }
                else {
                DosKey = msg.d;
            ...
            */
                ...
                var CSNRExist = false;
                $.ajax({
                    url: "VisitorRawCard.aspx/CheckCSNR",
                    type: "POST",
                    dataType: "json",
                    data: "{'CSNR':'" + escape(cardCSNR) + "'}",
                    async: false,
                    contentType: "application/json; charset=utf-8",
                    success: function (msg) {
                        if (msg.d == "True") {
                            ...
                            //alert(msg.d);
                            alert("Please add card in inventory.");
                            CSNRExist = false;
                            return false;
                        }
                    },
                    error: function () { alert('Internal server error, please try again later.'); alert(arguments[2]); }
                });
                ...
                /* ***************************************** Key Generation without Employee Type Start **********************************************/
                $.ajax({
                    url: "SACRawCard.aspx/GenerateKeyWithoutEmployeeType",
                    type: "POST",
                    dataType: "json",
                    data: "{'CSNR':'" + cardCSNR + "'}",
                    async: false,
                ...
                /* ***************************************** Key Generation without Employee Type End **********************************************/

                data = objReadCard.Authenticate(128, KeyA, 96);
                if (data != "") {
                    data = objReadCard.Authenticate(128, "FFFFFFFFFFFF", 96);
                    if (data != "") {
                        alert("Unable to Authenticate Block 128");
                        return false;
                    }
                }
                data = objReadCard.LoadKey(KeyA);
                if (data != "") {
                    alert("Unable to Load key to Block 128");
                    return false;
                }
                data = objReadCard.ReadBlock(128).replace(/ /g, "");
                var EmpType = data.substr(6, 2);

                var EmpCodePrefix = data.substr(0, 4);
                var EmpCodeNumber = data.substr(4, 10);
                ...
                $.ajax({
                    ...
                    type: "POST",
                    dataType: "json",
                    data: "{'CSNR':'" + cardCSNR + "','EMPCODE':'" + HexToASCII(EmpCodePrefix) + HexToASCII(EmpCodeNumber) + "'}",
                    async: false,
                    contentType: "application/json; charset=utf-8",
                    success: function (msg) {
                        //alert(msg.d);
                ...
                $.ajax({
                    url: "SACRawCard.aspx/GenerateKey",
                    type: "POST",
                    dataType: "json",
                    data: "{'CSNR':'" + cardCSNR + "','EMPCODE':'" + HexToASCII(EmpCodePrefix) + HexToASCII(EmpCodeNumber) + "'}",
                    async: false,
                ...
                data = objReadCard.Authenticate(16, MasterKey, 96);
                if (data != "") {
                ...
                data = objReadCard.Authenticate(17, MasterKey, 96);
                if (data != "") {
                    data = objReadCard.Authenticate(17, "FFFFFFFFFFFF", 96);
                    if (data != "") {
                        alert("Unable to Authenticate Block 17");
                        return false;
                    }
                }
                WriteData = PadBytes("00000000000000000000000000000000", 32);
                WriteData = objReadCard.Convert(WriteData);
                data = objReadCard.WriteData(WriteData, "11");
                if (data != "") {
                    alert("Card Error: Unable to write in the card.Sector 4 Block 17");
                    return false;
                }
                data = objReadCard.Authenticate(18, MasterKey, 96);
                if (data != "") {
                    data = objReadCard.Authenticate(18, "FFFFFFFFFFFF", 96);
                    if (data != "") {
                        alert("Unable to Authenticate Block 18");
                        return false;
                    }
                }
                WriteData = PadBytes("00000000000000000000000000000000", 32);
                WriteData = objReadCard.Convert(WriteData);
                data = objReadCard.WriteData(WriteData, "12");
                if (data != "") {
                    alert("Card Error: Unable to write in the card.Sector 4 Block 18");
                    return false;
                }
                data = objReadCard.Authenticate(19, MasterKey, 96);
                if (data != "") {
                ...
                data = objReadCard.Authenticate(41, DosKey, 96);
                if (data != "") {
                    data = objReadCard.Authenticate(41, "FFFFFFFFFFFF", 96);
                    if (data != "") {
                        alert("Unable to Authenticate Block 41");
                        return false;
                    }
                }
                WriteData = PadBytes("00000000000000000000000000000000", 32);
                WriteData = objReadCard.Convert(WriteData);
                data = objReadCard.WriteData(WriteData, "29");
                if (data != "") {
                    alert("Card Error: Unable to write in the card.Sector 10 Block 41");
                    return false;
                }
                data = objReadCard.Authenticate(42, DosKey, 96);
                if (data != "") {
                    data = objReadCard.Authenticate(42, "FFFFFFFFFFFF", 96);
                    if (data != "") {
                        alert("Unable to Authenticate Block 42");
                        return false;
                    }
                }
                WriteData = PadBytes("00000000000000000000000000000000", 32);
                WriteData = objReadCard.Convert(WriteData);
                data = objReadCard.WriteData(WriteData, "2A");
                if (data != "") {
                    alert("Card Error: Unable to write in the card.Sector 10 Block 42");
                    return false;
                }
                data = objReadCard.Authenticate(43, DosKey, 96);
                if (data != "") {
                ...
                /****************************************Sector 13 Start *********************************************/
                data = objReadCard.Authenticate(52, DosKey, 96);
                if (data != "") {
                ...
                data = objReadCard.Authenticate(53, DosKey, 96);
                if (data != "") {
                ...
                if (data != "") {
                    alert("Card Error: Unable to write in the card.Sector 13 Block 53");
                    return false;
                }
                data = objReadCard.Authenticate(54, DosKey, 96);
                if (data != "") {
                ...
                if (data != "") {
                    alert("Card Error: Unable to write in the card.Sector 13 Block 54");
                    return false;
                }
                data = objReadCard.Authenticate(55, DosKey, 96);
                if (data != "") {
                ...
                if (data != "") {
                    alert("Card Error: Unable to write in the card.Sector 10 Block 55");
                    return false;
                }
                /****************************************Sector 13 End *********************************************/
                ...
                // ***********************************Sector 32, 33, 34, 35 Start ************************************************//

               
                var Sector = 32;
                for (var i = 0, j = 128; i < 16 && Sector < 36; i++, j++) {
                    if (i == 15) {
                        data = objReadCard.Authenticate(j, KeyB, 96);
                        if (data != "") {
                ...
                // ***********************************Sector 32, 33, 34, 35 End ************************************************//

                data = objReadCard.Authenticate(124, KeyB , 96);
                if (data != "") {
                    data = objReadCard.Authenticate(124, "FFFFFFFFFFFF", 96);
                    if (data != "") {
                        alert("Unable to Authenticate Block 124");
                        return false;
                    }
                }
                WriteData = "000000";
                ...
                WriteData = objReadCard.Convert(WriteData);
                data = objReadCard.WriteData(WriteData, "7C");
               // alert("00"+data+"00");
                if (data != "") {
                    alert("Card Error: Unable to write in the card.Sector 31 Block 124");
                    return false;
                }

                data = objReadCard.Authenticate(127, KeyB , 96);
                if (data != "") {
                    data = objReadCard.Authenticate(127, "FFFFFFFFFFFF", 96);
                    if (data != "") {
                        alert("Unable to Authenticate Block 127");
                        return false;
                    }
                }
                WriteData = PadBytes("FFFFFFFFFFFFFF078069FFFFFFFFFFFF", 32);
                //alert(WriteData);
                WriteData = objReadCard.Convert(WriteData);
                data = objReadCard.WriteData(WriteData, "7F");
              //  alert("00" + data + "00");
                if (data != "") {
                    alert("Card Error: Unable to write in the card.Sector 31 Block 127");
                    return false;
                }
    ...
    <asp:Table ID="tblHead" runat="server" HorizontalAlign="Center" Width="100%">
        <asp:TableRow>
            <asp:TableCell HorizontalAlign="Center" CssClass="CompulsaryLabel">
                <asp:Label ID="lblHead" runat="server" Text="Reinitialise / Disable Card" ForeColor="RoyalBlue" Font-Size="20px"
                    Width="100%" CssClass="heading">
                </asp:Label>
            </asp:TableCell>
        </asp:TableRow>
    </asp:Table>
    <asp:Panel ID="pnlPersonalisation" runat="server" Style="width: 100%; text-align: center; margin-top:50px; ">
        <table style="margin: 0 auto;">
            <tr>
                <td colspan="2" class="heading" style="font-size: 18px;">
