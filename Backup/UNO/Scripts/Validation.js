...
function IsAlphanumeric(evnt) {
    ...
    if ((charCode >= 65 && charCode <= 90) || (charCode >= 48 && charCode <= 57) ||
			(charCode >= 97 && charCode <= 122) || (charCode == 8) || (charCode == 45) || (charCode == 32)) {
        return true
    }
    else {
        return false
...
function EntitySpecificDetails(strVal, strcomany, strlocation, strdivision, strdepartment, strcategory, LstCompany, LstLocation, LstDivision, LstDepartment, LstCategory, RbdEmp, RbdCompany, RbdLocation, RbdDivision, RbdDepartment, RbdCategory, btnOK, Btntd, Button4, LstEntitySelected, LstEmployee, txtSearchEmp, txtCompany, txtDivision, txtLocation, txtDepartment, txtShift, lblAvailableData, blSeletcedData, lstDCompany, lstDLocation, lstDDivision, lstDDepartment, lstDCategory, btnSave, lstEmployeeDummy) {
    ...
    $.ajax({
        ...
        dataType: "json",
        timeout: 10000,
        data: "{'strcom':'" + escape(strcom) + "','strloc':'" + escape(strloc) + "','strdiv':'" + escape(strdiv) + "','strdept':'" + escape(strdept) + "','strcat':'" + escape(strcat) + "','strchecked':'" + escape(strchecked) + "','strshift':'" + escape(strShift) + "','strSec':'" + escape(strSec) + "','strGRD':'" + escape(strGrade) + "'}",
        async: false,
...
function RptEntitySpecificDetails(strVal, strEmployee, strcomany, strlocation, strdivision, strdepartment, strShift, strSection, strGrade, LstEmployee, LstCompany, LstLocation, LstDivision, LstDepartment, LstCategory, LstSection, LstGrade, DEmployee, DCompany, DLocation, DDivision, DDepartment, DCategory, DSection, DGrade, RbdEmp, RbdCompany, RbdLocation, RbdDivision, RbdDepartment, RbdShift, RbdSection, RbdGrade, btnOK, btnCancel, txtSearchEmp, txtCompany, txtDivision, txtLocation, txtDepartment, txtShift, txtSection, txtGrade, Panel1, txtCalenderform, btnView, btnReset, HeadLbl, txtReader, ddlPersonalType) {
