   ...
   var sample = findSample(control, currentSample, categories), name = sample, parentId = null;
    ...
    function ViewModel() {
        ...
        self.loadLeftTab = function (divid) {
            ...
            var element = $("#samplesDiv>#" + divid + " .firstlevelback:visible").clone();
            $(element).appendTo("#dashboardheader").attr("id", "currentheader").addClass("dashboardheader");
            $("#samplesDiv>#" + divid + " .firstlevelback:visible").hide();
            if ($("#dashboardheader .current_control").length > 0)
                $("#dashboardheader .current_control").remove();
            $($("#samplesDiv>#" + divid + " .current_control").clone()).insertAfter("#currentheader").attr("id", "current_control");
        ...
        if(SamplesList.length >0)
        {
            ...
            foldername = SamplesList[0].name.toString().toLowerCase();
   ...
   if(links[0].href.indexOf("Content") >0) url =links[0].href.substr(0,links[0].href.indexOf("Content"));
    ...
    window.setTimeout(function () {
        ...
        if (waitingPopup.length > 0) {
            var waitingObj = waitingPopup.data("ejWaitingPopup");
            $(".scrollit").scroll(function () {
                if (waitingObj.model.showOnInit) {
...
function viewdemo(args) {
    ...
    if(window.location.pathname.match("sfejwebsamplebrowser")){       
        if (product == "analytics") window.open("http://localhost/sfejwebanalyticssamplebrowser"); else return false;        
    }    
